...
(function(){
  ...
  window.renderNav=function(opts){
    ...
    if(role==='sportive') sub='<nav class="subnav spo">'+SPO_GROUPS.map(([g,items])=>'<span class="grp"><span class="lab">'+g+'</span>'+items.map(([l,h])=>pill(l,h,'spo',here===l)).join('')+'</span>').join('<span class="sep"></span>')+'</nav>';
